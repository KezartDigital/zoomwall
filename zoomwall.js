...
export function create(blocks, enableKeys = false) {
    const imgs = blocks.querySelectorAll(".case");
    resize([...imgs]);
    blocks.classList.remove("loading");
    // shrink blocks if an empty space is clicked
    blocks.addEventListener("click", function () {
        const imgs = blocks.getElementsByClassName("case");
        if (imgs.length > 0) {
            console.log(imgs[0]);
            shrink(imgs[0]);
        }
...
function shrink(block) {
    ...
    if (blocks) {
        ...
        // reset all blocks
        blocks.querySelectorAll(".case").forEach(function (img) {
            reset(img);
        });
    }
}
...
function page(blocks, isNext = true) {
    const actives = blocks.querySelectorAll(".active");
    if (actives && actives.length > 0) {
        ...
        const imgs = wall.querySelectorAll(".case");
